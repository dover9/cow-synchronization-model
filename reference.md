...
$$
\dot{x}_i = \left( \alpha^{(i)}(\theta_i) + \frac{\sigma_y}{k_i} \chi_{\mathcal{R}}(\theta_j) \right) 
$$

<!-- $$
\dot{x}_i = \left( \alpha^{(i)}(\theta_i) + \frac{\sigma_x}{k_i} \sum\limits_{j=1}^{n} a_{ij} \chi_{\mathcal{E}}(\theta_j) \right) x_i
$$ -->
